...
const DrawingInterface = () => {
  ...
  // const [selectedBackground, setSelectedBackground] = useState<string>("train station");
  // const [selectedFood, setSelectedFood] = useState<string>("cake");
  
  // 배경과 음식 옵션
  const backgroundOptions = ["train station", "classroom", "street", "cafe", "restaurant", "park"];
  const foodOptions = ["cake", "pizza", "hamburger", "ramen", "sushi"];
  const colorPresets = [
    ...
    try {
      const response = await fetch('http://localhost:8000/generate', {
        ...
        headers: {
          ...
          'Accept': 'image/*'  // 이미지만 받도록 Accept 헤더 추가
        },
        body: JSON.stringify({
          // prompt: `score_9, score_8_up, score_7_up, scenery, ${selectedBackground}, ${selectedFood}`,
          prompt: prompt,
          loras: [
            {
              name: "zy_Detailed_Backgrounds_v1.safetensors",
              model_weight: 1.0,
              clip_weight: 1.0
            },
            {
              name: "yummyfood.safetensors",
              model_weight: 1.0,
              clip_weight: 1.0
            }
          ],
...
<div className="mb-6">
  <label className="text-sm font-medium mb-2 block">프롬프트 입력</label>
  <textarea
    value={prompt}
    onChange={(e) => setPrompt(e.target.value)}
    placeholder="원하는 이미지를 자세히 설명해주세요. (예: score_9, score_8_up, score_7_up, scenery, train station, cake)"
    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
    rows={4}
  />
  <p className="mt-2 text-xs text-gray-500">
    추천 프롬프트: score_9, score_8_up, score_7_up, scenery, [원하는 배경], [원하는 음식]
  </p>
</div>
Background & Food Selection
<div className="mb-6">
  <div className="mb-4">
    {/* <label className="text-sm font-medium mb-2 block">배경 선택</label>
    <select
  ...
  <div className="mb-4">
    {/* <label className="text-sm font-medium mb-2 block">음식 선택</label>
    <select
      value={selectedFood}
  ...
  <div className="text-xs text-gray-500">
    현재 프롬프트: score_9, score_8_up, score_7_up, scenery, {prompt}
  </div>
</div>
        {/* Color Section */}
        <div className="mb-6">
      ...
      <div className="w-64 bg-white p-4 shadow-lg">
        ...
        <div className="space-y-4">
          {['BREAK, rustic kitchen, soft light, highly detailed, environment, scenery, 3d style, cartoon', 'A large group of hikers gathered on a mountain trail, some resting, others taking photos of the beautiful landscape. People wear hiking gear', 'space background, galaxy, space shuttle', 'pizza, pasta, food'].map((example, index) => (
            <div
              key={index}
